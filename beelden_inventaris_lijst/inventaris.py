...
def set_dynamic_fields(result, csv_row):
    ...
    if dynamic.get('dc_creators'):
        ...
        csv_row['dc_creators_fotograaf'] = dynamic['dc_creators'].get(
            'Fotograaf')
    ...
    if dynamic.get('dc_titles'):
        ...
        csv_row['dc_titles_deelarchief'] = dynamic['dc_titles'].get(
            'deelarchief')
...
def csv_cols():
    return [
        'instelling', 'inventaris_nr', 'fragment_id', 'copyright',
        'vervaardiger1', 'vervaardiger2', 'vervaardiger3', 'vervaardiger4',
        'vervaardiger5', 'vervaardiger6', 'vervaardiger7',
        'dc_title', 'dc_source', 'dc_rechtenstatus', 'dc_creators', 'dc_rights_credit',
        'dc_creators_maker', 'dc_creators_fotograaf', 'dc_identifier_localid', 'dc_pid',
        'dc_titles_archief', 'dc_titles_deelarchief', 'li_persistente_uri_werk',
        'li_workpid', 'li_inventarisnummer', 'li_afbeelding', 'li_objectnaam',
        'li_persistente_uri_record', 'file_size', 'mimetype',
        'height', 'width', 'orientation', 'creation_date',
        'author', 'original_filename', 'title'
    ]
...
def process_inventaris_excel(mh_api, filename, outputfile):
    print(f"Reading {filename} and writing to {outputfile}")
    SKIP_ROWS = 1
    ...
    for ws in wb:
        for row in ws.rows:
            ...
            print(
                ...
                " procent processed=",
                round(float(count)/float(ws.max_row)*100, 2),
                "\n"
            )
...
def main():
    ...
    # this only finds 28 more entries by escaping (we set 'true' in mediahaven_api for escaping
